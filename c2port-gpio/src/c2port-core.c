...
static int c2port_read_sfr(struct c2port_device *dev, u8 sfr, u8 *data)
{
	c2port_write_ar(dev, sfr);
	if (c2port_read_dr(dev, data) < 0)
                return -EIO;

        return 0;
}

static int c2port_write_sfr(struct c2port_device *dev, u8 sfr, u8 data)
{
	c2port_write_ar(dev, sfr);
	if (c2port_write_dr(dev, data) < 0)
                return -EIO;
...
{
	u8 addr;
	int ret, timeout = 1000; /* erase flash needs long time... */
						...
						int status)
{
	...
	ret = c2port_write_dr(dev, 0xA5);
	if (ret < 0)
		return ret;

	/* Write the second keycode to enable C2 Flash programming */
	ret = c2port_write_dr(dev, 0xF1);
	if (ret < 0)
		return ret;
