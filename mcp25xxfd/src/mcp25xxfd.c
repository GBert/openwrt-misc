...
static int mcp25xxfd_sync_transfer(struct spi_device *spi,
				   ...
				   unsigned int xfers, int speed_hz)
{
	int i;
...
static int mcp25xxfd_write_then_read(struct spi_device *spi,
				     ...
				     unsigned int tx_len,
				     void *rx_buf,
				     unsigned int rx_len, int speed_hz)
{
...
static int mcp25xxfd_write(struct spi_device *spi,
			   const void *tx_buf,
			   unsigned int tx_len, int speed_hz)
{
	struct spi_transfer xfer;
...
static int mcp25xxfd_write_then_write(struct spi_device *spi,
				      ...
				      unsigned int tx_len,
				      const void *tx2_buf,
				      unsigned int tx2_len, int speed_hz)
{
	struct mcp25xxfd_priv *priv = spi_get_drvdata(spi);
	struct spi_transfer xfer;
...
static int mcp25xxfd_first_byte(u32 mask)
{
	return (mask & 0x0000ffff) ?
	    ((mask & 0x000000ff) ? 0 : 1) : ((mask & 0x00ff0000) ? 2 : 3);
}

static int mcp25xxfd_last_byte(u32 mask)
{
	return (mask & 0xffff0000) ?
	    ((mask & 0xff000000) ? 3 : 2) : ((mask & 0x0000ff00) ? 1 : 0);
}
...
static int mcp25xxfd_get_opmode(struct spi_device *spi, int *mode, int speed_hz)
{
	struct mcp25xxfd_priv *priv = spi_get_drvdata(spi);
	int ret;
	...
	ret = mcp25xxfd_cmd_read_mask(spi,
				      CAN_CON,
				      &priv->regs.con,
				      CAN_CON_OPMOD_MASK, speed_hz);
	if (ret)
		return ret;
	/* calculate the mode */
	*mode = (priv->regs.con & CAN_CON_OPMOD_MASK) >> CAN_CON_OPMOD_SHIFT;
...
static int mcp25xxfd_set_opmode(struct spi_device *spi, int mode, int speed_hz)
{
	struct mcp25xxfd_priv *priv = spi_get_drvdata(spi);
	...
	priv->regs.con = val |
	    (mode << CAN_CON_REQOP_SHIFT) | (mode << CAN_CON_OPMOD_SHIFT);
	priv->active_can_mode = mode;
...
static int mcp25xxfd_wake_from_sleep(struct spi_device *spi)
{
	...
	ret = mcp25xxfd_cmd_write(spi, MCP25XXFD_OSC,
				  priv->regs.osc, priv->spi_setup_speed_hz);
	if (ret)
		return ret;
	...
	dev_err(&spi->dev, "Clock did not enable within the timeout period\n");
	return -ETIMEDOUT;
}
...
static int mcp25xxfd_stop_clock(struct spi_device *spi, int requestor_mask)
{
	...
	mcp25xxfd_set_opmode(spi, CAN_CON_MODE_SLEEP, priv->spi_setup_speed_hz);
...
static void mcp25xxfd_mcpid_to_canid(u32 mcpid, u32 mcpflags, u32 * id)
{
	...
	if (mcpflags & CAN_OBJ_FLAGS_IDE) {
		*id = (eid << CAN_EFF_EID_SHIFT) |
		    (sid << CAN_EFF_SID_SHIFT) | CAN_EFF_FLAG;
	} else {
		*id = sid;
	}
...
static void __mcp25xxfd_stop_queue(struct net_device *net, unsigned int id)
{
...
static int mcp25xxfd_fill_spi_transmit_fifos(struct mcp25xxfd_priv *priv)
{
	...
	for (i = 0; i < priv->fifos.tx_fifos; i++) {
		...
		mcp25xxfd_calc_cmd_addr(INSTRUCTION_WRITE,
					FIFO_DATA(fifo_address), txm->fill_cmd);
...
static bool mcp25xxfd_is_last_txfifo(struct spi_device *spi, int fifo)
{
	struct mcp25xxfd_priv *priv = spi_get_drvdata(spi);

	return (fifo == (priv->fifos.tx_fifo_start + priv->fifos.tx_fifos - 1));
}
					...
					struct net_device *net)
{
	...
	if (can_is_canfd_skb(skb))
		...
		ret = mcp25xxfd_transmit_message(spi, fifo, (struct can_frame *)
						 skb->data);
					...
					int start, int end)
{
	...
	base = (priv->fifos.payload_mode << CAN_FIFOCON_PLSIZE_SHIFT) | ((priv->fifos.rx_fifo_depth - 1) << CAN_FIFOCON_FSIZE_SHIFT) | CAN_FIFOCON_RXTSEN |	/* RX timestamps */
	    CAN_FIFOCON_UINC | CAN_FIFOCON_TFERFFIE |	/* FIFO Full */
...
static int mcp25xxfd_read_fifos(struct spi_device *spi)
{
	...
	for (i = priv->fifos.rx_fifo_start + priv->fifos.rx_fifos - 1;
		...
		ret = mcp25xxfd_cmd_readn(spi,
					  ...
					  rx,
					  fifo_min_size, priv->spi_speed_hz);
		if (ret)
			return ret;
...
static int mcp25xxfd_bulk_read_fifos(struct spi_device *spi)
{
	...
	for (i = priv->fifos.rx_fifo_start + priv->fifos.rx_fifos - 1;
	     mask && (i >= priv->fifos.rx_fifo_start); i--) {
...
static void mcp25xxfd_mark_tx_processed(struct spi_device *spi, int fifo)
{
	struct mcp25xxfd_priv *priv = spi_get_drvdata(spi);
...
static int mcp25xxfd_can_ist_handle_tefif_handle_single(struct spi_device *spi)
{
	...
	ret = mcp25xxfd_cmd_readn(spi, FIFO_DATA(priv->fifos.tef_address), tef,
				  /* we do not read the last byte of the ts
				   * to avoid MAB issiues
				   */
				  sizeof(*tef) - 1, priv->spi_speed_hz);
	/* increment the counter to read next */
	ret = mcp25xxfd_cmd_write_mask(spi,
				       CAN_TEFCON,
				       CAN_TEFCON_UINC,
				       CAN_TEFCON_UINC, priv->spi_speed_hz);
						...
						int fifo)
{
	...
	ret = mcp25xxfd_cmd_read(spi,
				 CAN_FIFOSTA(fifo), &val, priv->spi_speed_hz);
	if (ret)
		return ret;
	...
	ret = mcp25xxfd_cmd_write_mask(spi,
				       ...
				       CAN_FIFOSTA_TXLARB |
				       CAN_FIFOSTA_TXERR |
				       CAN_FIFOSTA_TXATIF, priv->spi_speed_hz);
	...
	val &= CAN_FIFOSTA_TXABT | CAN_FIFOSTA_TXLARB | CAN_FIFOSTA_TXERR;
	switch (val) {
...
static int mcp25xxfd_can_ist_handle_cerrif(struct spi_device *spi)
{
	...
	if (priv->status.bdiag1 & (CAN_BDIAG1_DBIT0ERR | CAN_BDIAG1_NBIT0ERR)) {
		priv->can_err_id |= CAN_ERR_BUSERROR;
		priv->can_err_data[2] |= CAN_ERR_PROT_BIT0;
		priv->bdiag1_clear_mask |= CAN_BDIAG1_DBIT0ERR |
		    CAN_BDIAG1_NBIT0ERR;
	}
	if (priv->status.bdiag1 & (CAN_BDIAG1_DBIT1ERR | CAN_BDIAG1_NBIT1ERR)) {
		priv->can_err_id |= CAN_ERR_BUSERROR;
		priv->can_err_data[2] |= CAN_ERR_PROT_BIT1;
		priv->bdiag1_clear_mask |= CAN_BDIAG1_DBIT1ERR |
		    CAN_BDIAG1_NBIT1ERR;
	}
	if (priv->status.bdiag1 & (CAN_BDIAG1_DSTUFERR | CAN_BDIAG1_NSTUFERR)) {
		priv->can_err_id |= CAN_ERR_BUSERROR;
		priv->can_err_data[2] |= CAN_ERR_PROT_STUFF;
		priv->bdiag1_clear_mask |= CAN_BDIAG1_DSTUFERR |
		    CAN_BDIAG1_NSTUFERR;
	}
	if (priv->status.bdiag1 & (CAN_BDIAG1_DFORMERR | CAN_BDIAG1_NFORMERR)) {
		priv->can_err_id |= CAN_ERR_BUSERROR;
...
static int mcp25xxfd_disable_interrupts(struct spi_device *spi, u32 speed_hz)
{
	struct mcp25xxfd_priv *priv = spi_get_drvdata(spi);
...
static int mcp25xxfd_enable_interrupts(struct spi_device *spi, u32 speed_hz)
{
	struct mcp25xxfd_priv *priv = spi_get_drvdata(spi);
	...
	priv->status.intf = CAN_INT_TEFIE |
	    ...
	    CAN_INT_MODIE |
	    CAN_INT_SERRIE |
	    CAN_INT_IVMIE | CAN_INT_CERRIE | CAN_INT_RXOVIE | CAN_INT_ECCIE;
	return mcp25xxfd_cmd_write(spi, CAN_INT, priv->status.intf, speed_hz);
}
...
static int mcp25xxfd_can_ist_handle_status(struct spi_device *spi)
{
	...
	const u32 clear_irq = CAN_INT_TBCIF |
	    CAN_INT_MODIF |
	    CAN_INT_SERRIF | CAN_INT_CERRIF | CAN_INT_WAKIF | CAN_INT_IVMIF;
	int ret;

	/* clear all the interrupts asap */
	ret = mcp25xxfd_cmd_write_mask(spi, CAN_INT,
				       priv->status.intf & (~clear_irq),
				       clear_irq, priv->spi_speed_hz);
	if (ret)
		return ret;
...
static int mcp25xxfd_do_set_nominal_bittiming(struct net_device *net)
{
	...
	return mcp25xxfd_cmd_write(spi, CAN_NBTCFG,
				   priv->regs.nbtcfg, priv->spi_setup_speed_hz);
}
...
static int mcp25xxfd_do_set_data_bittiming(struct net_device *net)
{
	...
	return mcp25xxfd_cmd_write(spi, CAN_DBTCFG,
				   priv->regs.dbtcfg, priv->spi_setup_speed_hz);
}
...
static int mcp25xxfd_hw_probe(struct spi_device *spi)
{
	...
	switch (priv->regs.osc & (MCP25XXFD_OSC_OSCRDY | MCP25XXFD_OSC_OSCDIS)) {
	...
	ret = mcp25xxfd_cmd_read(spi, CAN_CON,
				 &priv->regs.con, priv->spi_setup_speed_hz);
	if (ret)
		return ret;
	...
	return mcp25xxfd_disable_interrupts(spi, priv->spi_setup_speed_hz);
}
...
static int mcp25xxfd_setup_osc(struct spi_device *spi)
{
	...
	dev_err(&spi->dev, "Clock did not lock within the timeout period\n");
				...
				struct spi_device *spi)
{
	...
	tx_memory_used = priv->fifos.tx_fifos *
	    (sizeof(struct mcp25xxfd_obj_tef) +
	     sizeof(struct mcp25xxfd_obj_tx) + priv->fifos.payload_size);
	...
	priv->fifos.rx_fifos = available_memory /
	    (sizeof(struct mcp25xxfd_obj_rx) +
	     priv->fifos.payload_size) / priv->fifos.rx_fifo_depth;
	...
	ret = mcp25xxfd_cmd_write(spi, CAN_TEFCON,
				  priv->regs.tefcon, priv->spi_setup_speed_hz);
	if (ret)
		return ret;

	/* set up tx fifos */
	val = CAN_FIFOCON_TXEN | CAN_FIFOCON_TXATIE |	/* show up txatie flags in txatif reg */
	    CAN_FIFOCON_FRESET |	/* reset FIFO */
	    (priv->fifos.payload_mode << CAN_FIFOCON_PLSIZE_SHIFT) | (0 << CAN_FIFOCON_FSIZE_SHIFT);	/* 1 FIFO only */
	...
	if (priv->can.ctrlmode & CAN_CTRLMODE_ONE_SHOT)
		...
		val |= CAN_FIFOCON_TXAT_UNLIMITED << CAN_FIFOCON_TXAT_SHIFT;
	...
	priv->fifos.tef_address_end = priv->fifos.tef_address_start +
	    priv->fifos.tef_fifos * sizeof(struct mcp25xxfd_obj_tef) - 1;
...
static int mcp25xxfd_setup(struct net_device *net,
			   struct mcp25xxfd_priv *priv, struct spi_device *spi)
{
	int ret;
	...
	priv->regs.ecccon = MCP25XXFD_ECCCON_ECCEN |
	    MCP25XXFD_ECCCON_SECIE | MCP25XXFD_ECCCON_DEDIE;
	ret = mcp25xxfd_cmd_write(spi, MCP25XXFD_ECCCON,
				  priv->regs.ecccon, priv->spi_setup_speed_hz);
	if (ret)
		return ret;
	...
	ret = mcp25xxfd_cmd_write(spi, CAN_TBC, 0, priv->spi_setup_speed_hz);
	if (ret)
		return ret;
	priv->regs.tscon = CAN_TSCON_TBCEN | ((priv->can.clock.freq / 1000000)
					      << CAN_TSCON_TBCPRE_SHIFT);
	ret = mcp25xxfd_cmd_write(spi, CAN_TSCON,
				  priv->regs.tscon, priv->spi_setup_speed_hz);
	if (ret)
		return ret;
...
static int mcp25xxfd_open(struct net_device *net)
{
	...
	ret = mcp25xxfd_enable_interrupts(spi, priv->spi_setup_speed_hz);
	if (ret)
		goto open_clean;
...
static void mcp25xxfd_debugfs_add(struct mcp25xxfd_priv *priv)
{
	...
	debugfs_create_u32("spi_setup_speed_hz", 0444, root,
			   &priv->spi_setup_speed_hz);
	debugfs_create_u32("spi_speed_hz", 0444, root, &priv->spi_speed_hz);
	...
	debugfs_create_u32("fifo_start", 0444, rx, &priv->fifos.rx_fifo_start);
	debugfs_create_u32("fifo_count", 0444, rx, &priv->fifos.rx_fifos);
	debugfs_create_x32("fifo_mask", 0444, rx, &priv->fifos.rx_fifo_mask);
	debugfs_create_u64("rx_overflow", 0444, rx, &priv->stats.rx_overflow);
	debugfs_create_u64("rx_mab", 0444, stats, &priv->stats.rx_mab);

	debugfs_create_u32("fifo_start", 0444, tx, &priv->fifos.tx_fifo_start);
	debugfs_create_u32("fifo_count", 0444, tx, &priv->fifos.tx_fifos);
	debugfs_create_x32("fifo_mask", 0444, tx, &priv->fifos.tx_fifo_mask);
	...
	debugfs_create_x32("fifo_processed", 0444, tx,
			   &priv->fifos.tx_processed_mask);
	debugfs_create_u32("queue_status", 0444, tx, &priv->tx_queue_status);
	debugfs_create_u64("tx_mab", 0444, stats, &priv->stats.tx_mab);

	debugfs_create_u32("tef_count", 0444, tx, &priv->fifos.tef_fifos);

	debugfs_create_u32("fifo_max_payload_size", 0444, root,
			   &priv->fifos.payload_size);

	/* interrupt statistics */
	debugfs_create_u64("int", 0444, stats, &priv->stats.irq_calls);
	debugfs_create_u64("int_loops", 0444, stats, &priv->stats.irq_loops);
	debugfs_create_u64("int_ivm", 0444, stats, &priv->stats.int_ivm_count);
	...
	debugfs_create_u64("int_ecc", 0444, stats, &priv->stats.int_ecc_count);
	debugfs_create_u64("int_tef", 0444, stats, &priv->stats.int_tef_count);
	debugfs_create_u64("int_mod", 0444, stats, &priv->stats.int_mod_count);
	debugfs_create_u64("int_tbc", 0444, stats, &priv->stats.int_tbc_count);
	debugfs_create_u64("int_rx", 0444, stats, &priv->stats.int_rx_count);
	debugfs_create_u64("int_tx", 0444, stats, &priv->stats.int_tx_count);
...
static int mcp25xxfd_can_probe(struct spi_device *spi)
{
	...
	if (ret == -ENODEV) {
		ret = mcp25xxfd_hw_probe(spi);
		if (!ret)
			dev_info(&spi->dev, "found device only during retry\n");
	}
	if (ret) {
		if (ret == -ENODEV)
			dev_err(&spi->dev,
